...
int setRBG(int sensorvalue,int colorstate){
 ...
 // float ivc = internal_voltage_converter;
  ...
  if (sensorvalue < color_multiple-color_dead_zone){
    setColor(0, 255, 0);  // red
    colorstate = 1;
  }
  else if (sensorvalue >= color_multiple + color_dead_zone && sensorvalue < color_multiple*2 - color_dead_zone){
    setColor(100,50,255); //red
    colorstate = 2;
  }
  else if (sensorvalue >= color_multiple*2 + color_dead_zone && sensorvalue < color_multiple*3 - color_dead_zone){
    setColor(120, 10, 255);  // yellow  
    colorstate = 3;
  }
  else if (sensorvalue >= color_multiple*3 + color_dead_zone && sensorvalue < color_multiple*4 - color_dead_zone){
    setColor(255, 0, 255);  // green
    colorstate = 4;
  }
  else if (sensorvalue >= color_multiple*4 + color_dead_zone && sensorvalue < color_multiple*5 - color_dead_zone){
    setColor(255, 255, 0);  // blue
    colorstate = 5;
  }
  else if (sensorvalue >= color_multiple*5 + color_dead_zone && sensorvalue < color_multiple*6 - color_dead_zone){
    setColor(200, 255, 0);  // purple
    colorstate = 6;
  }
  else if (sensorvalue >= color_multiple*6 + color_dead_zone && sensorvalue < color_multiple*7 - color_dead_zone){
    setColor(180, 50, 100);  // white
    colorstate = 7;
  }
  else if (sensorvalue >= color_multiple*7 + color_dead_zone ){
...
void fast_mode_check(bool switch_state){
  float temp_multiplier = 0.0;
  
  if(real_mode_speed_up == 0){
    //fast_mode_state = switch_state;
    real_mode_speed_up =1; //aka do once
    
    if(switch_state){
      temp_multiplier = 1.0 / (float)real_mode_speed_factor;
    }else{
      temp_multiplier = (float)real_mode_speed_factor;
    }
  }
  ...
  if(switch_state != fast_mode_state){
    fast_mode_state = switch_state;
    
    prox_millis *= temp_multiplier;
    prox_off_time *= temp_multiplier;
    prox_speed_up *= temp_multiplier;
    burst_rate *= temp_multiplier;
    data_millis *= temp_multiplier;
  }
}
